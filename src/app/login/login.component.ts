...
export class LoginComponent {
  ...
  revealPassword(event: Event, subID: string): void {
    ...
    const reveal: HTMLImageElement = document.getElementById("pswd_reveal_img" + subID) as HTMLImageElement;
    const hide: HTMLImageElement = document.getElementById("pswd_hide_img" + subID) as HTMLImageElement;
    const pswdInput: HTMLInputElement  = document.getElementById("pswd_input" + subID) as HTMLInputElement;
  ...
  checkEmail(email: string ) {
    if (email.length == 0) {
      const emailInput: HTMLInputElement = document.getElementById("register_email_input") as HTMLInputElement;
      emailInput.style.borderColor ="#8f8f8f";
      return;
    }
    if (validate(email)) {
      this.displayMessage("register_email_input", "Please use a valid email address!", false);
      this.emailValid = true;
    } else {
      this.displayMessage("register_email_input");
  ...
  checkCredentials(): void {
    ...
    if (this.loginPswd.length > 0 && this.email.length > 0) {
  ...
  checkPswd(registerPswd: string): void {
    const pswdInput: HTMLInputElement = document.getElementById("pswd_input_reg") as HTMLInputElement;
    const criterias: Array<[string,  boolean]> = [
      ["#pswd_spaces", registerPswd.includes(" ") == false],
